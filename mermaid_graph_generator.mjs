...
if (process.argv.length !== 3) {
	console.error(
		'example usage: node mermaid_graph_generator.mjs chains/rich_hunters.json',
	);
	exit(1);
}
...
const transitions = Object.keys(eventChain.events).map((e) => ({
	...
	reachable: [
		...
		...(eventChain.events[e].choices || [])
			.flatMap((choice) => choice.next)
			.map((next) => next.event),
	],
}));

const nodeLines = transitions
	.map((transition) => {
...
const arrowLines = transitions
	.filter((node) => node.reachable.length !== 0)
	.map(
		(node) =>
			`${node.event.replace('end', 'End')} --> ${node.reachable.map((event) => event.replace('end', 'End')).join(' & ')}`,
	)
	.map((l) => '\t' + l);

console.log(['flowchart TD', ...nodeLines, ...arrowLines].join('\n'));
