...
function main() {
	...
	chainFiles
		.map((fileName) => collectTranslationTagsOfChain(fileName))
...
function updateTranslationTagsOfChain(chainFileName) {
	...
	for (const event in eventChain.events) {
		...
		if (eventChoices) {
			for (let i = 0; i < eventChoices.length; i++) {
				...
				lines.push({
					tag,
					text: removeTag(eventChain.events[event].choices[i].text),
				});
			}
		}
	}
	...
	locales.forEach((locale) => {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			const alreadyTranslatedText = alreadyTranslatedContent.find(
				(line) => line.tag === tag,
			)?.text;
...
function readTranslationFile(tsvFileName) {
	...
	return readFileSync(tsvFileName)
		.toString()
		.split('\n')
		.map((line) => {
			const s = line.split('\t');
