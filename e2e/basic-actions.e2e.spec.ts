...
test('an outcome with a truthy condition should be displayed when condition is matched', async ({page}) => {
    const testedChain = 'a_simple_chain_with_hidden_and_not_outcomes_conditioned_on_pop';
    ...
    await test.step('When there is 2 pop we should get the corresponding event', async () => {
        ...
        await expect(page.getByText('outcome_above_and_eq_3_pop')).not.toBeVisible();
    });
