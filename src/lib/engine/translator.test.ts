...
import { extractFileInfo, parseFileContent, parseLine, translate, type TranslationStore } from './translator';
...
describe('translate', () => {
    ...
    it('should return the translated version of a line if it exists', () => {
        ...
        expect(translate(text, 'fr_FR', mockTranslations)).toEqual('Cependant, un rapport se distingue:')
    });

    it('should return the original version of a line if the requested locale is not available', () => {
        const text = 'One report catches your attention though: #ho65md';
        expect(translate(text, 'it_IT', mockTranslations)).toEqual('One report catches your attention though:')
    });

    it('should return the original version of a line if the requested line is not translated for the requested locale', () => {
        const text = 'Praise them for being so skilled at “tactical retreat” #4nz2up';
        expect(translate(text, 'fr_FR', mockTranslations)).toEqual('Praise them for being so skilled at “tactical retreat”')
    });
});
