...
describe('engine test', () => {
    ...
    it('a chain depending on ecology should not be available before ecology is unlocked', () => {
        const engine = new Engine(new ChainStore({overrideInputChains: getChainFromDisk('chain_with_ecology_dependency.json')}));
    ...
    it('a chain depending on population should be available because population is always unlocked', () => {
        const engine = new Engine(new ChainStore({overrideInputChains: getChainFromDisk('chain_with_population_dependency.json')}));
